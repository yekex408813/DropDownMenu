...
public class SingleGridView<DATA> extends GridView implements AdapterView.OnItemClickListener {
    ...
    private void init(Context context) {
        ...
        setNumColumns(4);//设置一行几个
        setBackgroundColor(Color.WHITE);
        setSmoothScrollbarEnabled(false);

        int topBottomDp = UIUtil.dp(context, 22);
        int leftRightDp = UIUtil.dp(context, 30);
        int vhDp = UIUtil.dp(context, 18);

        setVerticalSpacing(vhDp);//设置子view的padding
        setHorizontalSpacing(vhDp);
        setPadding(leftRightDp, topBottomDp, leftRightDp, topBottomDp);
